...
func SealVMessAEADHeader(key [16]byte, data []byte) []byte {
	...
	binary.Write(lengthbuf, binary.BigEndian, HeaderDataLen)
	...
	outPutBuf.Write(authid[:]) //16

	outPutBuf.Write(authidCheck) //16

	outPutBuf.Write(lengthbufb) //2

	outPutBuf.Write(nonce) //8

	outPutBuf.Write(headerSealed)
...
func OpenVMessAEADHeader(key [16]byte, authid [16]byte, data io.Reader) ([]byte, bool, error, int) {
	...
	binary.Read(bytes.NewReader(lengthbufb[:]), binary.BigEndian, &length)
